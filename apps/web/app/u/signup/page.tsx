...
function Signup() {
    ...
    return (
        <>
            ...
            <button
                className="w-full bg-blue-500 hover:cursor-pointer hover:bg-blue-600 text-white px-4 py-2 font-bold rounded"
                onClick={() => signIn("google", { callbackUrl: "/auth-callback?mode=signup" })}
            >
                Signup with Google
            </button>
            <div className="text-center font-semibold">already have an account? <button className="text-blue-300 hover:underline hover:cursor-pointer" onClick={() => router.push("/u/login")}>Login</button></div>
            {/* </div> */}
            {/* </div> 
            </div> */}
        </>
